...
class Dataset:
	def __init__(self,w,h,batch_size=100,dataset_size=1000,average_sequence_length=5000,interactive=False,max_speed=3,brown_damping=0.9995,brown_velocity=0.005,init_velocity=0,init_rho=None,n_cond=False,dt=1,types=["magnus","box","pipe"],images=["cyber","fish","smiley","wing"],background_images=["empty"], for_data_generation = False):
		"""
		create dataset
		:w: width of domains
		:h: height of domains
		...
		:for_data_generation: if True, the dataset is used for input data generation for the training of RL agents.
		"""
		...
		self.n_cond = n_cond
		self.for_data_generation = for_data_generation
		if n_cond:
	...
	def reset_env(self,index):
		...
		if type == "magnus": # magnus effect (1)
		...
		if True:
			np_to_be_saved = [self.cond_mask[index],self.v_cond[index]]
			np_to_be_saved = torch.cat(np_to_be_saved,dim=0).cpu().numpy()
			# np.save(f"input_{index}.npy",np_to_be_saved)
			import matplotlib.pyplot as plt
			plt.imsave(f"input_0.png",np_to_be_saved[0,:,:])
			plt.imsave(f"input_0_u.png",np_to_be_saved[1,:,:])
			plt.imsave(f"input_0_v.png",np_to_be_saved[2,:,:])
			pass
